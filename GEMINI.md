...
- **Tests live under `experiments/test_*.py`** and are pytest-discoverable. 58 tests covering `data_mix`, `thinking` (GRPO advantages), `eval_callback`, `sft_code` (think-burst insertion), `activation_checkpointing` (bit-identical fwd/bwd), and `eos_mask`. Run with `PYTHONPATH=. .venv/bin/python -m pytest experiments/test_*.py -v`. Add a test when you add a new builder, filter, or shaping mode.
- **Trainer module layout** (`experiments/train_lm.py` was split for readability, 2026-05-13):
  ...
  - `experiments/optim_utils.py` — `build_optimizer(model, optimizer=..., lr=..., lr_muon=..., alpha_wd=..., steps=...)` returns `(opts, scheds)`. Handles Muon + AdamW split + FiLM-α param-group split.
  - `experiments/speed_knobs.py` — `apply_speed_knobs(model, bf16=True, tf32=True)` does the bf16-autocast wrap + TF32. Call AFTER model construction.
