"""
Phase 21d — eval the 708 M K=3-self-feeding-trained sparse-(2, 34) FiLM model.

Adapted from `eval_filmed_ppl_217m.py`. Architecture parameters are read
from the checkpoint config (d_model=1024, n_heads=16, d_head=64,
n_layers=36, feedback_pairs=((2,34),), feedback_self_k=3 expected).

Three quantities are reported on a deterministic 32 K-token codeparrot val
slice — the same `train.shuffle(seed=42).skip(10_000)`, T=512, batch 4
slice that produced the 708 M std-2-pass numbers (PPL 34.26 / 36.97):
  ...
  1. **Training-protocol PPL** — runs `model(x)` which executes the K-iter
     ...
     previous iter's source-layer output).

  2. **2-pass PPL** (gold reference) — disables self-feeding for the
     forward and runs the standard 2-pass FiLM (pass-1 vanilla, pass-2
     with lag-1 of pass-1 source). This is the protocol whose lagged-
     cached drift the std-2-pass 708 M ckpt failed (+7.9 %).
  ...
  3. **Lagged-cached deployment PPL** — token-by-token streaming forward
     ...
     step's pass-2 source-layer output as a lag-1 proxy. This is the 1×
     decode cost protocol. The whole point of K=3 self-feeding is to
     close the gap between this number and the 2-pass number.

Self-consistency check: ‖prev_iter.source - final.source‖ averaged over
a val batch, normalised by ‖final.source‖. A small relative norm (< ~5 %)
indicates the iteration has converged and lagged-cached deployment
will track the training distribution.

Outputs JSON with all three PPLs, the self-consistency norm, and gaps.

Usage:
    CUDA_VISIBLE_DEVICES=0 ./.venv/bin/python experiments/eval_filmed_ppl_708m.py \
...
# ---------------------------------------------------------------------------
# Loader for 708M sparse-FiLM checkpoint, including possible self-feeding
# config field. Architecturally identical to load_film_217m — the 708M
# config is read from the checkpoint dict, so the same loader works.
# ---------------------------------------------------------------------------


def load_film_ckpt(path: str, device: str = "cuda",
                    override_self_k: int | None = None) -> TinyLM:
    """Load a TinyLM sparse-FiLM checkpoint, optionally overriding the
...
# ---------------------------------------------------------------------------
# Val tail — same exact 32 K-token slice the 708M std-2-pass parity
# check used in commit d2876e3. Reuses TokenisedStream from train_lm.py
...
@torch.no_grad()
def eval_model_forward_ppl(model, chunks: torch.Tensor, batch: int = 2) -> tuple:
...
def lagged_cached_logits_one_seq(model, tokens: torch.Tensor,
    ...
    lagged-cached protocol. Identical to the 217M version. Returns logits
    (1, T, V).
...
def eval_self_consistency(model, chunks: torch.Tensor,
    ...
    for i in range(0, N, batch):
        ...
        x = batch_chunks[:, :-1]
        _ = model(x)
        prev = model._last_self_feed_prev_src    # iter K-2 (no_grad'd)
...
def main():
    ...
    p.add_argument("--ckpt", type=str, required=True,
                   help="Path to the 708M sparse-FiLM checkpoint.")
    ...
    p.add_argument("--out", type=str, default="bench_film_self_k3_708M_ppl.json")
    ...
    model_self = load_film_ckpt(args.ckpt, device=args.device)
    cfg_d = {
    ...
    # produce a coherent PPL when run in 2-pass mode, and gives the gold
    # reference for the lagged-cached comparison.
    print("\n--- 2-pass PPL (gold reference, training-faithful 2-pass) ---")
    del model_self
    torch.cuda.empty_cache()
    model_2pass = load_film_ckpt(args.ckpt, device=args.device, override_self_k=0)
    t0 = time.perf_counter()
