...
DONOR = "Qwen/Qwen2.5-Coder-0.5B-Instruct"
TOKID = "Qwen/Qwen2.5-Coder-0.5B-Instruct"   # donor's own Qwen tokenizer (~151936 vocab)
DEV = "cuda"

# Qwen2.5-Coder-0.5B architecture (verified from AutoConfig). DONOR arch;
# the STUDENT may keep fewer layers (depth-reduction, see resolve_layer_select).
# NOTE base vs instruct: same architecture/vocab; instruct is cached + measured
# (96/164 HumanEval). For the real heal a base donor is cleaner (we re-SFT), but
# the smoke only validates copy mechanics so either works.
ARCH = dict(
    vocab_size=151936, d_model=896, n_layers=24, n_heads=14, d_head=64,
    d_ff=4864, rms_eps=1e-6, tie_embeddings=True,
)
...
# (n_layers_out == donor depth, no explicit list) preserves the original
# full-depth path byte-for-byte. The default selection for any n_layers_out is
# evenly spaced, always including the first (0) and last (donor_L-1) donor
# layers.
...
# --------------------------------------------------------------------------- #
def _evenly_spaced_layer_select(n_layers_out: int, donor_L: int) -> list[int]:
    """Evenly-spaced donor-layer indices, always including layer 0 and
    donor_L-1 (first + last)."""
    if n_layers_out <= 1:
        return [donor_L - 1]
    return sorted({round(i * (donor_L - 1) / (n_layers_out - 1))
                   for i in range(n_layers_out)})


# Default 10-layer selection, computed from the donor's ACTUAL depth
# (ARCH["n_layers"]). This used to be a hardcoded list copied from a 32-layer
# donor script; when the donor became Qwen2.5-Coder-0.5B (24 layers) the list
# still contained indices up to 31, silently out of range for a 24-layer
# donor. Computing it here keeps it correct if ARCH ever changes again.
DEFAULT_LAYER_SELECT_10L = _evenly_spaced_layer_select(10, ARCH["n_layers"])
...
def resolve_layer_select(n_layers_out: int, explicit: str | None) -> list[int]:
    ...
    (an evenly-spaced selection computed from the donor's actual depth);
    else evenly-spaced incl. first+last, computed the same way.
    """
    ...
    if explicit:
        ...
        sel = _evenly_spaced_layer_select(n_layers_out, donor_L)
...
def save_ckpt(model, path, step, stage, n_layers=None, layer_select=None):
    ...
    cfg = dict(arch="deltanet", n_layers=n_layers, d_model=ARCH["d_model"],
               ...
               donor=DONOR, tokenizer=TOKID, layer_select=layer_select)
...
def main():
    ...
    # --- depth-reduction (default: full-depth donor-layer-count identity = original behaviour) ---
    ap.add_argument("--n_layers_out", type=int, default=ARCH["n_layers"],
                    help=f"student layer count; <{ARCH['n_layers']} (the donor "
                         f"depth) keeps a subset of donor layers")
