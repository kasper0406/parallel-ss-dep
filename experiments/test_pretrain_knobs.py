...
def _run_grad_accum_equivalence(output_gate: bool):
    ...
    _, _, lm_loss, aux_loss, _ = _nonthink_forward_loss(m_full, x, y, args, 0, None)
    ...
    for i in range(n_micro):
        _, _, lm_loss_i, aux_loss_i, _ = _nonthink_forward_loss(
...
def test_gate_entropy_aux_off_returns_zero():
    ...
    y = torch.randint(0, vocab, (B, T))
    m = _MockLM(vocab, d)
    _, _, _, _, gate_aux = _nonthink_forward_loss(
...
def test_gate_entropy_aux_pushes_gate_against_uncertainty():
    ...
    _, _, lm_loss, _, gate_aux = _nonthink_forward_loss(m, x, y, args, 0, None)
