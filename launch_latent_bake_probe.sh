#!/bin/bash
# Latent-reasoning BAKE probe (2026-06-05) — the reviewer's gated first experiment.
#
# Continue the 287M Phase-C base with the depth-matched latent-REASONING co-train
# (answer-span CE on pointer-chase, R=depth, curriculum, clean latent thread)
# REPLACING the broken general-text latent_cotrain. Run as an A/B:
#   RWEIGHT=0.05 TAG=latent_bake_on   GPU=0  -> with reasoning co-train
#   RWEIGHT=0.0  TAG=latent_bake_ctrl GPU=1  -> identical control, no reasoning
# Go/no-go: (a) WITH-reasoning VAL must track the control VAL (no general
# regression), (b) reason(loss) drops + a pointer-chase none-vs-R lift emerges.
#
# Arch flags mirror launch_pretrain_phase_c.sh so the ckpt loads byte-compatibly.
# PKM bootstrap curricula are OFF (epsilon/alpha_floor start 0) — the PKM is
# already mature in phase_c; re-injecting exploration would disrupt it.
# --no-compile because the latent reasoning loop runs extra eager forwards.
set -e
...
GWEIGHT=${GWEIGHT:-0.0}     # >0 also trains the gate to invoke+halt thinking
TAG=${TAG:-latent_bake_on}
STEPS=${STEPS:-30000}
...
CUDA_VISIBLE_DEVICES=${GPU:-0} nohup .venv/bin/python -u experiments/train_lm.py \
    ...
    --pkm_diversity_weight 0.01 --pkm_alpha_floor_start 0.0 \
    ...
    --tb_dir runs/tb/${TAG} \
    > runs/${TAG}.log 2>&1 &
echo "Launched ${TAG} (RWEIGHT=${RWEIGHT}) on GPU ${GPU:-0} (PID $!)"
