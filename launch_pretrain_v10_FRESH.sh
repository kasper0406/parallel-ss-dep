...
CUDA_VISIBLE_DEVICES=${GPU:-1} nohup .venv/bin/python -u experiments/train_lm.py \
    ...
    --save_ckpt checkpoints/pretrain_v10_fresh.pt \
    --tb_dir runs/tb/pretrain_v10_fresh \
    > runs/pretrain_v10_fresh.log 2>&1 &
echo "Launched v10 all-features pretrain on GPU ${GPU:-1} (PID $!)"
echo "Watch:  tail -f runs/pretrain_v10_fresh.log"
echo "Per-feature load-bearing signal: grep '\\[feature-probe\\]' runs/pretrain_v10_fresh.log"
