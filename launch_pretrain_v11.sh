...
CUDA_VISIBLE_DEVICES=${GPU:-0} nohup .venv/bin/python -u experiments/train_lm.py \
    ...
    --save_ckpt checkpoints/pretrain_v11.pt \
    --tb_dir runs/tb/pretrain_v11 \
    > runs/pretrain_v11.log 2>&1 &
echo "Launched v11 pretrain on GPU ${GPU:-0} (PID $!)"
echo "Watch:  tail -f runs/pretrain_v11.log"
echo "WM load-bearing signal: grep -E '\\[feature-probe\\]|\\[wm-recall\\]' runs/pretrain_v11.log"
