#!/bin/bash
# v18 — RESUME the v17 all-features run from its latest checkpoint onto the
# arXiv-augmented data mix (2026-06-18). User: add arXiv (full text + fresh
# abstracts), then "restart the current training run at the next checkpoint".
#
# This is a DATA-MIX SWAP via checkpoint-resume, NOT a from-scratch run:
#   * loads the v17 mid-eval ckpt (model + optimizer + step) → --load_ckpt/--start_step
#   * swaps --data_mix to configs/pretrain_mix_v18_arxiv.yaml (= v14 maskfix + arXiv)
#   * writes NEW checkpoints under the pretrain_v18 tag so the v17 lineage is a
#     preserved fallback (the v17 ckpts are NOT overwritten).
# Streaming data is not checkpointed (fresh shuffle), so the mix swap is clean.
# All curricula (latent@2000, PKM floor@3000, FiLM-k@1300) are already past at the
# resume step, so they stay engaged. Same --steps 19000 → WSD decay lands as planned.
# REQUIRED env: CKPT=<v17 ckpt path>  START=<its step>.   (set them before running)
set -uo pipefail
...
GPU=${GPU:-0}
STEPS=${STEPS:-19000}
TAG=${TAG:-pretrain_v18}
: "${CKPT:?set CKPT=checkpoints/pretrain_v17_stepNNNN_tok....pt}"
: "${START:?set START=<the ckpt step int>}"

CUDA_VISIBLE_DEVICES=$GPU PYTHONUNBUFFERED=1 nohup .venv/bin/python -u experiments/train_lm.py \
  --load_ckpt "$CKPT" --start_step "$START" \
  ...
  > "runs/${TAG}.log" 2>&1 &
echo "launched v18 arXiv-resume on GPU $GPU (PID $!) from $CKPT @ step $START -> runs/${TAG}.log"
