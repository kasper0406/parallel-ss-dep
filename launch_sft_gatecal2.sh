...
CUDA_VISIBLE_DEVICES=${GPU:-0} nohup .venv/bin/python -u experiments/sft_code.py \
    ...
    --save_ckpt checkpoints/sft_gatecal.pt \
    ...
    --mem_size 1024 \
    --gate_calibration_weight 0.1 \
    --gate_calibration_sigma_low 0.1 --gate_calibration_sigma_high 0.9 --gate_calibration_max_positions 16 --gate_calibration_sample_frac 0.05 \
    --epochs 2 \
    --batch 4 \
    --lr 3e-5 \
    --max_len 1024 \
    --log_every 100 \
    > runs/sft_gatecal.log 2>&1 &
echo "Launched combined SFT on Phase C base, GPU ${GPU:-0} (PID $!)"
echo "Watch: tail -f runs/sft_gatecal.log"
