...
# ast-clean (parse+has-def) distill corpus. The ONLY difference vs sft_phase_c_combined.pt
...
if [ ! -f data/distill_v7_phase1_astclean.jsonl ]; then
  echo "ERROR: data/distill_v7_phase1_astclean.jsonl missing (the ast-clean (parse+has-def) distill)."; exit 1
fi
cat data/distill_v7_phase1_astclean.jsonl \
    data/synthetic_memory.jsonl \
    data/longctx_recall.jsonl \
    > data/sft_phasec_astclean.jsonl
echo "[sft-phasec-clean] corpus rows: $(wc -l < data/sft_phasec_astclean.jsonl) (clean-distill+synthmem+longctx)"

CUDA_VISIBLE_DEVICES=${GPU:-1} nohup .venv/bin/python -u experiments/sft_code.py \
    --load_ckpt checkpoints/pretrain_phase_c.pt \
    --save_ckpt checkpoints/sft_phasec_astclean.pt \
    --distilled_jsonl data/sft_phasec_astclean.jsonl \
    --with_thinking \
    ...
    --max_len 1024 \
    --log_every 100 \
    > runs/sft_phasec_astclean.log 2>&1 &
echo "Launched clean-corpus SFT on Phase C base, GPU ${GPU:-1} (PID $!)"
echo "Watch: tail -f runs/sft_phasec_astclean.log"
